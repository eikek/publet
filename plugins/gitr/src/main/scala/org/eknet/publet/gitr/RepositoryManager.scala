...
import org.eknet.publet.actor.messages.GetPartition
import org.eknet.publet.actor.docroot.PartitionActor
...
class RepositoryManager extends Actor with Logging {
  ...
  def receive = {
    case GetPartition(uri) => {
      ...
      sender ! Some(context.actorOf(Props(new PartitionActor(p)), name = "gitpart"+partCount.getAndIncrement))
    }
